...
async function loadActiveCategories() {
    try {
        const data = await apiRequest("/api/v1/category/all-active")
        if (data) {
            allCategories = data || []
        }
    } catch (error) {
...
async function loadAllCategories() {
    try {
        const [activeData, inactiveData] = await Promise.all([
            apiRequest("/api/v1/category/all-active"),
            apiRequest("/api/v1/category/all-inactive"),
        ])

        allCategories = [...(activeData || []), ...(inactiveData || [])]
...
async function loadActiveProducts() {
    try {
        const data = await apiRequest("/api/v1/product/all-active")
        if (data) {
            allProducts = data || []
        }
    } catch (error) {
...
async function loadAllProducts() {
    try {
        const data = await apiRequest("/api/v1/product/admin")
        if (data) {
            allProducts = data || []
            renderProductsTable()
        }
...
function renderCategoriesTable() {
    ...
    tbody.innerHTML = allCategories
        ...
        <tr class="fade-in">
            ...
            <td>${category.name}</td>
            <td>
                <span class="status-badge ${category._active ? "active" : "inactive"}">
                    ${category._active ? "Faol" : "Nofaol"}
                </span>
            </td>
            <td>
        ...
        .map((product) => {
            ...
            <tr class="fade-in">
                ...
                product.attachments && product.attachments.length > 0
                    ? `<img src="${API_BASE_URL}/api/v1/attachment/${product.attachments[0].id}" class="image-preview" alt="${product.name}" onerror="this.src='/placeholder.svg?height=50&width=50'">`
                    : '<i class="fas fa-image text-muted"></i>'
            }
                </td>
                <td>${product.name}</td>
...
async function handleCategorySubmit(e) {
    ...
    try {
        ...
        await apiRequest("/api/v1/category/", {
            method: "POST",
            headers: {
...
async function handleProductSubmit(e) {
    ...
    try {
        ...
        await apiRequest("/api/v1/product/admin", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
...
function editCategory(categoryId) {
    ...
    document.getElementById("edit-category-active").checked = category._active
...
async function updateCategory() {
    ...
    try {
        await apiRequest(`/api/v1/category/${id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
...
async function deleteCategory(categoryId) {
    ...
    try {
        await apiRequest(`/api/v1/category/${categoryId}`, {
            method: "DELETE",
        })
...
async function updateProduct() {
    ...
    try {
        ...
        await apiRequest(`/api/v1/product/admin/${id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
...
async function deleteProduct(productId) {
    ...
    try {
        await apiRequest(`/api/v1/product/admin/${productId}`, {
            method: "DELETE",
        })
